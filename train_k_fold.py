...
#         if not moved_bags:
#             print("No valid bags in batch, skipping...")
#             continue
...
def main(config):
    ...
    #     print("Removing bad image 9491446_R from test set")
    #     test_pids.remove("9491446_R")
    
    # if "9491446_R" in train_val:
    #     print("Removing bad image 9491446_R from train_val set")
    #     idx = np.where(train_val == "9491446_R")[0][0]
    #     train_val = np.delete(train_val, idx)
    #     train_val_grades = np.delete(train_val_grades, idx)
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(train_val, train_val_grades)):
        ...
        #     list_of_patch_bags, labels_batch, group_names, features_batch = batch
        #     for i, bag in enumerate(list_of_patch_bags):
        #         print(f"Batch {batch_idx}, Bag {i}, num patches: {len(bag)}")
        #     break
